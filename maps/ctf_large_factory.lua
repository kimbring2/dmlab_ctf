...
function factory.createLevelApi(kwargs)
  ...
  function api:playerModel(playerId, playerName)
    ...
    if playerId == 1 then
      return "crash_color"
    elseif playerId == 2 then
      return 'crash_color/skin' .. 1
    elseif playerId == 3 then
      return 'crash_color/skin' .. 2
    elseif playerId == 4 then
      return 'crash_color/skin' .. 2
    end
  ...
  local function updateSkinCustom(playerSkin, team)
    --print("updateSkinCustom")
    --print("team: ", team)
  ...
  function api:modifyTexture(name, texture)
    --print("api:modifyTexture()")
    --print("name: ", name)

    local _, _, id = string.find(name,'models/players/crash_color/skin_base(%d*).tga')
    if id then
      if id == '' then
        id = 1
      else
      ...
      --print("id: ", id) 
      local rgb = api._botColors[1]

      if id == 2 then
        updateSkinCustom(texture, 1)
      elseif id == 3 then
        updateSkinCustom(texture, 2)
      end
