...
class A3CAgent:
    ...
    def step(self, action, env):
        reward = env.step(ACTIONS[ACTION_LIST[action]], num_steps=self.action_repeat)
    ...
    def train(self, n_threads):
        ...
        envs = [deepmind_lab.Lab("ctf_simple", ['RGB_INTERLEAVED', 'DEBUG.GADGET_AMOUNT', 'DEBUG.GADGET', 'DEBUG.HAS_RED_FLAG'],
                {'fps': str(self.fps), 'width': str(self.width), 'height': str(self.height)}) for i in range(n_threads)]
    ...
    def train_threading(self, agent, env, thread):
        while self.episode < self.EPISODES:
            ...
            state_screen, state_inv = self.reset(env)
            ...
            while not done:
                ...
                action, memory_state, carry_state = agent.act(np.reshape(state_screen, (1,*self.screen_size)), 
                ...
                next_screen_state, next_inv_state, _, done, _ = self.step(action, env)
                events = env.events()
